...
const VerifyOTPForm = (props: Props) => {
  ...
  return (
    <div className='card-sign-in pt-5'>
      <h2>Enter passcode</h2>
      <p className={styles.smsInstructions}>
        A 6-digit passcode was sent to you at <strong>{email}</strong>.
      </p>
      <form onSubmit={onSubmit}>
        <div className={`${styles.passcodeContainer} mt-5`}>
        ...
        <div className={styles.resendCodeContainer}>
          <p className={`${styles.resendCodeText} mt-5`}>Didn’t get it? <span className='resend-button' onClick={resendCode}>Resend Code</span> </p>
        </div>
        <input className={'btn btn-primary'} disabled={isDisabled} id="button" type="submit" value="Continue" />
      </form>
    </div>
  );
};
